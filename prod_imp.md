...
- **PE5** `test_cold_start_under_500ms` stabilized against WSL2/NTFS long-tail variance via two complementary mechanisms: (a) `_wsl2_active()` detector + 2.5x `_COLD_START_MULT` (1.25s budget on WSL2, 0.5s elsewhere), and (b) **median-of-3** sampling inside the test (matches U26's subprocess-test stability pattern — a +200ms regression moves the median, not just the tail, so we keep regression signal).  Budget alone couldn't stabilize: 1.5x flaked ~70% on the reference host, 2.5x still flaked ~10% on 10-run repeats against pure p100 comparison; median-of-3 on top closed the gap.  `test_version_subprocess_cold_start` (U26) likewise measures median across 3 subprocess invocations.
- **PE6** `MAX_ALLOWED_SECONDS` (module-level steady-state budget used by `TestLargeDatabase` / `TestLargeInventory` / `TestLargePolicies`) gains a 6x multiplier under xdist parallel mode (`_parallel_mode_active()` gate), giving 1.8s instead of 0.3s when running `pytest -n auto`.  On WSL2 with 8 workers contending for NTFS, SQLite WAL + syscall latency pushes steady-state timings past 0.3s (observed: 0.32s, 0.80s, 0.98s on three bellwether tests).  6x (1.8s) gives ~80% headroom above observed p100; 3/3 consecutive full-suite parallel runs pass clean.  WSL2 serial keeps the strict 0.3s guard.  Native Linux/macOS CI uses the same multiplier cosmetically — tests complete in 0.05-0.1s there regardless — so the authoritative perf gate on native hosts is serial-mode's 0.3s.
