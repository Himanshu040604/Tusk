...
| `rewrite_changes` | integer | when rewrite emitted | Count of `RewriteChange` records produced by the rewriter (i.e. `len(rewrite_result.changes)`). The full per-change array — with `change_type`, `description`, `original_value`, `new_value`, `statement_index`, `confidence` (float, default `1.0`; `< 0.5` triggers `LOW_CONFIDENCE` self-check finding), and `rationale` (string or null) — is emitted by the standalone `sentinel rewrite --output-format json` command (`JsonFormatter.format_rewrite_result`), not by the pipeline JSON above. Bundle E N2 added the `confidence`/`rationale` fields; Bundle F M2 / Bundle G N4 corrected this row to match the pipeline emitter. |
| `self_check` | object | when emitted | Pipeline JSON emits `verdict` (PASS/WARNING/FAIL), `completeness_score` (float), `findings_count` (int) per `JsonFormatter.format_pipeline_result`. The full `SelfCheckResult` (with `iterations`, per-check breakdown, suppressed list) is the standalone `sentinel self-check` formatter, not the pipeline emitter. |
